...
import com.shopping.util.PropertyMgr;
...
public class SalesCountServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		JFreeChart chart = ChartFactory.createBarChart3D("product selling graph", // 图表标题
				"product", // 目录轴的显示标签
				"selling", // 数值轴的显示标签
				dataset, // 数据集
...
System.out.println(PropertyMgr.getProperty("statImagePath"));
	...
	private CategoryDataset getDataSet() {
		...
		for (int i = 0; i < items.size(); i++) {
			...
			dataset.addValue(p.getTotalSalesCount(), "salecount", p.getProductName());
		}
		return dataset;
	}
